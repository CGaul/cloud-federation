...
import agents.{DependencyState, FederationBroker, MatchMakingAgent}
 ...
 */
class MatchMakingSystemTest (_system: ActorSystem) extends TestKit(_system)
	with WordSpecLike with Matchers with BeforeAndAfterAll with GivenWhenThen {

/* Global Values: */
/* ============== */

  private val cloudConfDir1 = new File ("Agent-Tests/src/test/resources/cloudconf1")
  private val cloudConfDir2 = new File ("Agent-Tests/src/test/resources/cloudconf2")
	private val cloudConfig1 = CloudConfigurator(cloudConfDir1)
	private val cloudConfig2 = CloudConfigurator(cloudConfDir2)
...
/* ========================= */
  ...
  override def beforeAll(): Unit = {
    require(cloudConfDir1.isDirectory, "Directory cloudconf1 needs to exist in \"Agent-Tests/src/test/resources/\"!")
    require(cloudConfDir2.isDirectory, "Directory cloudconf2 needs to exist in \"Agent-Tests/src/test/resources/\"!")
  ...
  /* The MMAActor Generation with NRA Test-Probe: */
  /* -------------------------------------------- */
...
//  private val nraName = "networkResourceAgent"
//  private val nraSel = system.actorSelection("akka://cloudAgentSystem/user/"+nraName)
  private val nraSel = system.actorSelection(localNRAProbe.ref.path)
  
  private val fedBrokerName = "federationBroker"
  private val fedBrokerSel = system.actorSelection("akka://cloudAgentSystem/user/"+fedBrokerName)
	
  private val mmaProps: Props = Props(classOf[MatchMakingAgent], cloudConfig1, fedBrokerSel, nraSel)
	private val localMMATestActor = TestActorRef[MatchMakingAgent](mmaProps, name = "matchMakingAgent")

//  private val nraProps: Props = Props(classOf[NetworkResourceAgent], cloudConfig1, fedBrokerSel, localMMATestActor)
//  private val localNRATestActor = TestActorRef[NetworkResourceAgent](nraProps, name = nraName)

  
  /* The Federation-Broker Actor Generation: */
  /* --------------------------------------- */
  
  private val fedBrokerProps: Props = Props(classOf[FederationBroker], fedConfig)
  private val fedBrokerTestActor = TestActorRef[FederationBroker](fedBrokerProps, name = fedBrokerName)
...
/* ==================== */
  ...
  val (resAlloc1, tenant1) = MatchMakingSystemTest.prepareTestResources(cloudConfig1)
  ...
  "A MatchMakingAgent" should {
    ...
    "send an OvxInstanceRequest to the FederationBroker, when becoming ONLINE" in{
      ...
      while(localMMATestActor.underlyingActor.state == DependencyState.OFFLINE) {
        Thread.sleep(100)
      }
...
//      localCCFMProbe.send(localMMATestActor, ResourceRequest(tenant1, resAlloc1))
//
