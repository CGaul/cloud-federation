...
/* ========== */
		...
		private var hostTopology: List[Host] = cloudHosts
...
/* =============== */
	...
	def active(): Receive = {
		case message: NRAResourceDest	=> message match {
			...
			case ResourceFederationSummary(resourcesAllocated)
			=> recvResourceFederationSummary(resourcesAllocated)
		}
...
//		log.info("Send json-Query {} to OVX Hypervisor", jsonQuery) TODO: delete
	 ...
	 */
	private def recvResourceFederationSummary(federatedResources: Map[ActorRef, ResourceAlloc]): Unit = {
	}
					    ...
					    case Some(portMap) =>
								...
								vHost match{
								    case Some(result)  => 
											...
											val portMapIndex = switchPortMap.get(physSwitch).get.indexOf(hostPortMap)
											switchPortMap = switchPortMap +
												(physSwitch -> switchPortMap.getOrElse(physSwitch, List()).updated(portMapIndex, newHostPortMap))
